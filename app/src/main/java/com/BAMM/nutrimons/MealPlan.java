...
import com.BAMM.nutrimons.database.Meal;
...
/*                ArrayList<String> finalMeals = new ArrayList<>();
                ...
                if (mDb.dateDataDao().findByDate(finalDateString) == null) {
                    final com.BAMM.nutrimons.database.DateData dateData = new com.BAMM.nutrimons.database.DateData(finalDateString, selectedBreakfast, selectedLunch, selectedDinner, selectedSnack, new ArrayList<>(), (float) 0.0, null, BAMM.MAX_DAILY_COINS);
                    ...
                    List<com.BAMM.nutrimons.database.Meal> breakfast;
                    try {
                        breakfast = temp.getBreakfast();
                    } catch (NullPointerException e) {
                        breakfast = null;
                    }
                    List<com.BAMM.nutrimons.database.Meal> lunch;
                    try {
                        lunch = temp.getLunch();
                    } catch (NullPointerException e) {
                        lunch = null;
                    }
                    List<com.BAMM.nutrimons.database.Meal> dinner;
                    try {
                        dinner = temp.getDinner();
                    } catch (NullPointerException e) {
                        dinner = null;
                    }
                    List<com.BAMM.nutrimons.database.Meal> snack;
                    try {
                        snack = temp.getSnack();
                    } catch (NullPointerException e) {
                        snack = null;
                    ...
                    if (breakfastSpinner.getSelectedItem().toString().equalsIgnoreCase("Select an item")) {
                        ...
                        if (breakfast != null) {
                            if (!breakfast.isEmpty()) {
                                for (com.BAMM.nutrimons.database.Meal b : breakfast) {
                                    if (b != null) {
                    ...
                    if (lunchSpinner.getSelectedItem().toString().equalsIgnoreCase("Select an item")) {
                        ...
                        if (lunch != null) {
                            if (!lunch.isEmpty()) {
                                for (com.BAMM.nutrimons.database.Meal l : lunch) {
                                    if (l != null) {
                    ...
                    if (dinnerSpinner.getSelectedItem().toString().equalsIgnoreCase("Select an item")) {
                        ...
                        if (dinner != null) {
                            if (!dinner.isEmpty()) {
                                for (com.BAMM.nutrimons.database.Meal d : dinner) {
                                    if (d != null) {
                    ...
                    if (snackSpinner.getSelectedItem().toString().equalsIgnoreCase("Select an item")) {
                        ...
                        if (snack != null) {
                            if (!snack.isEmpty()) {
                                for (com.BAMM.nutrimons.database.Meal s : snack) {
                                    if (s != null) {
                    ...
                    final com.BAMM.nutrimons.database.DateData dateData = new com.BAMM.nutrimons.database.DateData(finalDateString,
